    ...
    <div class="content-grow">
        ...
        <div class="container my-5">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <div>
                    <h1 class="main-title mb-1">Daftar Kelas</h1>
                    <p class="text-white-50 mb-0">Kelola dan lihat seluruh data kelas aktif </p>
                </div>
                <div>

        <!-- LOGIKA TOMBOL TAMBAH -->
            @if(auth()->user()->role === 'admin')
    <a href="{{ action([App\Http\Controllers\KelasController::class, 'create']) }}" class="btn btn-create btn-lg">+ Tambah Kelas</a>
@else
    <button type="button" class="btn btn-create btn-lg" data-bs-toggle="modal" data-bs-target="#adminOnlyModal" data-fitur="menambah data kelas">+ Tambah Kelas</button>
@endif
        </div>
    </div>
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $c->id }}</td>
<td class="fw-semibold text-white">{{ $c->kode_kelas }}</td>
                <td><span class="badge bg-secondary text-white px-2 py-1">{{ $c->dosen?->fullname ?? 'Dosen Tidak Ditemukan'  }}</span></td>
...
<div class="d-flex justify-content-center gap-2">
                  <!-- LOGIKA TOMBOL DELETE -->
                               @if(auth()->user()->role === 'admin')
    <form action="{{ action([App\Http\Controllers\KelasController::class, 'destroy'], [$c->id]) }}" method="post" class="m-0">
        @csrf
        <input type="hidden" name="_method" value="DELETE">
        <button type="submit" class="btn btn-action-delete">Delete</button>
    </form>
@else
    <button type="button" class="btn btn-action-delete" data-bs-toggle="modal" data-bs-target="#adminOnlyModal" data-fitur="menghapus data kelas">Delete</button>
@endif
                            </div>
                        </td>
                    </tr>
